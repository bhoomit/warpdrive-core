...
var Hyperlapse = function(container, params) {
	...
	_ptime = 0, _dtime = 0,
	_prev_pano_id = null,
	_raw_points = [], _h_points = [], response = null, _actual_total_distance = 0, _max_points_modified = false,
    _retries = 0;
...
var handleRouteComplete = function (e) {
    if(self.length() == 0 || (_retries >= 5 && self.length() < 100)){
        console.log("ERROR:: " + title + " :: Got " + self.length() + " points, max retry exceeded");
        handleError({'message': "Couldn't get enough frames"});
        return;
    }else if(_retries >= 5 && self.length() >= 100){
        console.log("DEBUG:: " + title + " :: Got " + self.length() + " points, no more retries procedding for pano loading");
    }else if(self.length() < 300){
        console.log("DEBUG:: " + title + " :: Got " + self.length() + " points so in creasing max_points");
        console.log("DEBUG:: " + title + " :: Current max_points " + _max_points);
		_max_points += 75 * _max_points/self.length();
        _max_points_modified = true;
        console.log("DEBUG:: " + title + " :: New max_points " + _max_points);
        self.reset();
        _retries += 1;
        handleDirectionsRoute(self.response);
        return;
    }else if(self._actual_total_distance > 7000 && self.length() < 350){
        console.log("DEBUG:: " + title + " :: Got " + self.length() + " points so in creasing max_points");
        console.log("DEBUG:: " + title + " :: Current max_points " + _max_points);
		_max_points += 75 * _max_points/self.length();
        _max_points_modified = true;
        ...
        handleDirectionsRoute(self.response);
        return;
    }else if(self.length() > 500){
        console.log("DEBUG:: " + title + " :: Got " + self.length() + " points so in decreasing max_points");
        console.log("DEBUG:: " + title + " :: Current max_points " + _max_points);
		_max_points -= 50;
        _max_points_modified = true;
        console.log("DEBUG:: " + title + " :: New max_points " + _max_points);
...
var actual_total_distance = total_distance;
self._actual_total_distance = total_distance;
if(total_distance > 7000 && !_max_points_modified) total_distance = total_distance*2/3;
if(actual_total_distance > 10000 && _max_points > 150 && !_max_points_modified) _max_points = 250;
...
for(i=0; i<path.length; i++) {
    console.log(total_distance - d_t + "::" + _d);
    if(Math.floor(total_distance/1000) > 400) {
        handleError({'message': "Route is too long :: " + total_distance/1000 + " KM"});
        return;
    }
	if(i+1 < path.length) {
		 if(total_distance > 3500 && (!seg1 || !seg2)){
			if(total_distance - d_t < 3100 && !seg2){
				seg2 = true;
				segment_length = ((total_distance-d_t)/3)/(_max_points*1/3);
				_d = (segment_length < _distance_between_points) ? _d = _distance_between_points : _d = segment_length;
			}else if(total_distance - d_t < 1100 && !seg1){
				seg1=true;
				segment_length = ((total_distance-d_t)/2)/(_max_points*1/3);
				_d = (segment_length < _distance_between_points) ? _d = _distance_between_points : _d = segment_length;
			}else if(d_t == 0){
				segment_length = (total_distance-d_t)/(_max_points*1/3);
				_d = (segment_length < _distance_between_points) ? _d = _distance_between_points : _d = segment_length;
			}
		}
		a = path[i];
		...
		if(!_max_points_modified && actual_total_distance > 15000 && Math.floor(d_t/1000) % 3 == 0){
			d_t += d
			continue;
		}
		if(r > 0 && r < d) {
...
this.garbageCollect = function(){
    delete _h_points[_point_index-3];
};
/**
